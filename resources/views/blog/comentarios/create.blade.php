...
@section('titulo', __('Crear Comentario'))
...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Crear Comentario') }}</div>
                            ...
                            <div class="col-md-6">
                                <select id="articulo_id" class="form-control @error('articulo_id') is-invalid @enderror" name="articulo_id" required>
                                    ...
                                    @foreach($articulos as $articulo)
                                        <option value="{{ $articulo->id }}" {{ old('articulo_id') == $articulo->id ? 'selected' : '' }}>{{ $articulo->titulo }}</option>
                                    @endforeach
                                </select>
                                @error('articulo_id')
                                    <span class="invalid-feedback" role="alert">
